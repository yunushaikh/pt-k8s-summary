...
type podLogRow struct {
	...
	Files                                  []podLogFile
	PodYAMLModalID string // id base for textarea (suffix -full); empty when no pods.yaml match
	PodYAMLEscaped string // HTML-escaped YAML for modal; empty when unavailable
}

func applyPodK8sToRows(rows []podLogRow, k8s map[string]jpreport.PodK8sMeta) {
	dash := "—"
	for i := range rows {
		r := &rows[i]
		k := r.Namespace + "\x00" + r.Pod
		if k8s != nil {
			if m, ok := k8s[k]; ok {
				r.Ready, r.Status, r.Restarts, r.Age, r.IP, r.Node = m.Ready, m.Status, m.Restarts, m.Age, m.IP, m.Node
				continue
			}
		}
		r.Ready, r.Status, r.Restarts, r.Age, r.IP, r.Node = dash, dash, dash, dash, dash, dash
	}
}
...
func gatherPodLogsSectionHTML(dumpRoot, galeraSince string, k8s map[string]jpreport.PodK8sMeta, pods *jpreport.PodLoader) (string, error) {
	...
	applyPodK8sToRows(rows, k8s)
	if pods != nil {
		for i := range rows {
			escY, mid, ok := pods.PodYAMLEscapedForModal(dumpRoot, rows[i].Namespace, rows[i].Pod)
			if ok {
				rows[i].PodYAMLModalID = mid
				rows[i].PodYAMLEscaped = escY
			}
		}
	}
	...
	b.WriteString(`<style>
#pxc-pod-logs .pxc-plg-note { font-size: 0.72rem; color: #64748b; margin: 0.9rem 0 0.75rem 0; line-height: 1.45; }
...
#pxc-pod-logs td.pxc-plg-stat { font-size: 0.75rem; white-space: nowrap; vertical-align: middle; }
#pxc-pod-logs td.pxc-plg-ns { max-width: 9rem; word-break: break-word; }
...
#pxc-pod-logs .pxc-plg-pod-yaml-btn { font: inherit; font-size: inherit; font-family: ui-monospace, Menlo, monospace; background: none; border: none; padding: 0; margin: 0; color: #0369a1; text-decoration: underline; text-underline-offset: 2px; cursor: pointer; text-align: left; }
#pxc-pod-logs .pxc-plg-pod-yaml-btn:hover { color: #0c4a6e; }
...
#pxc-pod-logs .pxc-gle-wrap { margin: 1.1rem 0 0 0; padding: 0.9rem 1rem; background: linear-gradient(165deg,#f8fafc 0%,#f1f5f9 100%); border: 1px solid #e2e8f0; border-radius: 12px; }
...
#pxc-pod-logs .pxc-gle-blob { display: none; }
	...
	for podIdx, row := range rows {
		...
		b.WriteString(`"><td class="pxc-plg-ns">`)
		b.WriteString(esc(row.Namespace))
		b.WriteString(`</td><td class="pxc-plg-mono pxc-plg-name">`)
		if row.PodYAMLEscaped != "" && row.PodYAMLModalID != "" {
			b.WriteString(`<button type="button" class="pxc-plg-pod-yaml-btn" data-target="`)
			b.WriteString(esc(row.PodYAMLModalID))
			b.WriteString(`" data-plg-pod-label="`)
			b.WriteString(esc(row.Namespace + " / " + row.Pod))
			b.WriteString(`" title="View pod YAML (from dump or merged pods.yaml)">`)
			b.WriteString(esc(row.Pod))
			b.WriteString(`</button>`)
		} else {
			b.WriteString(esc(row.Pod))
		}
		b.WriteString(`</td><td class="pxc-plg-stat">`)
		b.WriteString(esc(row.Ready))
		b.WriteString(`</td><td class="pxc-plg-stat">`)
		b.WriteString(esc(row.Status))
		b.WriteString(`</td><td class="pxc-plg-stat">`)
		b.WriteString(esc(row.Restarts))
		b.WriteString(`</td><td class="pxc-plg-stat">`)
		b.WriteString(esc(row.Age))
		b.WriteString(`</td><td class="pxc-plg-stat pxc-plg-mono">`)
		b.WriteString(esc(row.IP))
		b.WriteString(`</td><td class="pxc-plg-stat">`)
		b.WriteString(esc(row.Node))
	...
	for _, row := range rows {
		if row.PodYAMLEscaped != "" && row.PodYAMLModalID != "" {
			b.WriteString(`<textarea readonly hidden class="pxc-plg-pod-yaml-store" id="`)
			b.WriteString(esc(row.PodYAMLModalID + "-full"))
			b.WriteString(`">`)
			b.WriteString(row.PodYAMLEscaped)
